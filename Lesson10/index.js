...
// 1.In ra danh sách các sản phẩm có price >=50
function PriceUp50(products){
  let total="";
  for(let i=0;i<products.length;i++){
    if(products[i].price>=50){
      total+=products[i].title
    }
  }
  return total
}
console.log(PriceUp50(products))
//2 In ra danh sách các sản phẩm có đánh giá từ 4 trở lên. (dựa vào giá trị "rate")
function RateUp4(products){
  let total_2="";
  for(let i=0;i<products.length;i++){
    if(products[i].rating.rate>4){
      total_2+=products[i].title  
    }
  }
  return total_2
}
console.log(RateUp4(products))
//3. In ra danh sách các sản phẩm thuộc category: "women's clothing"
function WomenClo(product){
  let total_3="";
  for(let i=0;i<products.length;i++){
    if(products[i].category==="women's clothing"){
      total_3+=product[i].title
    }
  }
  return total_3
}
